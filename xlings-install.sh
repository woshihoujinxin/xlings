...
SOFTWARE_URL="https://github.com/woshihoujinxin/xlings/archive/refs/heads/dev.zip"
...
SOFTWARE_URL2="https://gitee.com/houjinxin/xlings/repository/archive/dev.zip"
...
# Ensure XLINGS_HOME_DIR is set and persisted
detect_shell_rc() {
  # macOS 默认使用 zsh；Linux 常用 bash
...
ensure_xlings_home_dir() {
  ...
  if ! grep -q 'XLINGS_HOME_DIR' "$rc_file"; then
    {
      echo ''
      echo '# xlings: install script added home dir'
...
# 继续原有行为：source bashrc（若是 zsh 环境，此处可能无效，但我们已在当前会话 export）
source ~/.bashrc || true
